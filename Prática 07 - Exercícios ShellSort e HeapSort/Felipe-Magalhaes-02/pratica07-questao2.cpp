...
void HeapRefaz(TNumero* num,int esq,int dir,int* comp,int* mov)
{
	int i=esq;
	int j=i*2+1;
	...
	{
		comp[8]+=2;
		if(j<dir && num[j].chave<num[j+1].chave)
		{
			j++;
		}
		if(aux.chave>=num[j].chave)
...
void HeapConstroi(TNumero* num,int n,int* comp,int* mov)
{
	int esq;
	esq=(n/2)-1;
	while(esq>=0)
	{
		HeapRefaz(num,esq,n-1,comp,mov);
		esq--;
	}
}

void HeapSort(TNumero* num,int n,int* comp,int* mov)
{
	TNumero aux;
	HeapConstroi(num,n,comp,mov);
	while(n>1)
	{
		mov[8]+=3;
		aux=num[n-1];
